...
// Outpoint defines a zua data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new zua transaction outpoint point with the
...
// TxIn defines a zua transaction input.
type TxIn struct {
...
// NewTxIn returns a new zua transaction input with the provided
...
// TxOut defines a zua transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new zua transaction output with the provided
...
// MsgTx implements the Message interface and represents a zua tx message.
