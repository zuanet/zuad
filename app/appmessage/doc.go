/*
Package appmessage implements the zua appmessage protocol.

At a high level, this package provides support for marshalling and unmarshalling
supported zua messages to and from the appmessage. This package does not deal
with the specifics of message handling such as what to do when a message is
received. This provides the caller with a high level of flexibility.

# Zua Message Overview

The zua protocol consists of exchanging messages between peers. Each
message is preceded by a header which identifies information about it such as
which zua network it is a part of, its type, how big it is, and a checksum
...
of the currently supported zua messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using zua encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the zua messages are intended
...
and write zua messages. The following sections provide a quick overview of
...
The zua network is a magic number which is used to identify the start of a
message and which zua network the message applies to. This package provides
...
As discussed in the zua message overview section, this package reads
and writes zua messages using a generic interface named Message. In
...
In order to unmarshall zua messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a zua peer. Example syntax is:
	...
	msg, rawPayload, err := appmessage.ReadMessage(conn, pver, zuaNet)
	if err != nil {
...
In order to marshall zua messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a zua peer. Example syntax to request addresses
from a remote peer is:
	...
	// error.
	err := appmessage.WriteMessage(conn, msg, pver, zuaNet)
	if err != nil {
		// Log and handle the error
	}
