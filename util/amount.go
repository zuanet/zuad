...
// other than the base unit of a zua. The value of the AmountUnit
// is the exponent component of the decadic multiple to convert from
// an amount in zua to an amount counted in units.
type AmountUnit int

// These constants define various units used when describing a zua
// monetary amount.
const (
...
// Amount represents the base zua monetary unit (colloquially referred
// to as a `Sompi'). A single Amount is equal to 1e-8 of a zua.
type Amount uint64
...
// some value in zua. NewAmount errors if f is NaN or +-Infinity, but
// does not check that the amount is within the total amount of zua
...
func NewAmount(f float64) (Amount, error) {
	...
	case math.IsInf(f, 1):
		fallthrough
	case math.IsInf(f, -1):
		return 0, errors.New("invalid zua amount")
	}

	return round(f * constants.SompiPerZua), nil
}

// ToUnit converts a monetary amount counted in zua base units to a
// floating point value representing an amount of zua.
...
// Format formats a monetary amount counted in zua base units as a
...
// useful for services that build on top of zua (for example, calculating
