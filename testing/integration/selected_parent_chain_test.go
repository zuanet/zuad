...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of zuad instances
	zuad1, zuad2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := zuad1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In zuad2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, zuad2)
	}

	// Connect the two zuads. This should trigger sync
	// between the two nodes
	connect(t, zuad1, zuad2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := zuad1.rpcClient.GetVirtualSelectedParentChainFromBlock(
