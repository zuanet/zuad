...
// 2. A wrapper that implements isZuadMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `ZuadMessage_` prefix and `Request` suffix
...
// generateZuadMessage generates a wrapped ZuadMessage with the given `commandValue`
func generateZuadMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.ZuadMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	zuadMessage := reflect.New(reflect.TypeOf(protowire.ZuadMessage{}))
	zuadMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return zuadMessage.Interface().(*protowire.ZuadMessage), nil
}
