...
import (
	...
	"github.com/zuanet/zuad/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		zuadMessage, err := client.Post(&protowire.ZuadMessage{Payload: &protowire.ZuadMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := zuadMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	zuadMessage := &protowire.ZuadMessage{}
	err := protojson.Unmarshal([]byte(response), zuadMessage)
	if err != nil {
	...
	return marshalOptions.Format(zuadMessage)
}
