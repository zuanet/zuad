...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("zuadsanity-zuad-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("zuad", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "ZUAD-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "ZUAD-STDERR")
