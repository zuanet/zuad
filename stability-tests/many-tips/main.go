...
import (
	...
	"github.com/zuanet/zuad/domain/consensus/utils/mining"
	...
	"github.com/zuanet/zuad/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("zuad-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("zuad datadir: %s", dataDir)

	zuadCmd, err := common.StartCmd("ZUAD",
		"zuad",
	...
	spawn("startNode-zuadCmd.Wait", func() {
		err := zuadCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("zuadCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes zuad closes ungracefully
				log.Errorf("zuadCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(zuadCmd, "zuadCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	zuaMinerCmd, err := common.StartCmd("MINER",
		"zuaminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("zua-miner-Cmd.Wait", func() {
		err := zuaMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(zuaMinerCmd, "zuaMinerCmd")
	return nil
}
